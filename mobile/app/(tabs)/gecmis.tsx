...
export default function HistoryScreen() {
      ...
      >
        ...
        <View style={styles.cardMiddle}>
          <View style={styles.cardPersonas}>
            {session.selectedCharacters.slice(0, 4).map((mbti) => {
              ...
              return (
                <View key={mbti} style={[styles.personaDot, { backgroundColor: vis?.accentColor || '#7C5CFC' }]} />
              );
            })}
            <Text style={styles.cardMeta}>
              {session.selectedCharacters.length} {t.history_voices} · {session.turns.length} {t.history_turns}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.cardBottom}>
          <View style={[
            ...
            <View style={[styles.statusDot, { backgroundColor: isComplete ? '#34D399' : '#7C5CFC' }]} />
            <Text style={[styles.statusLabel, { color: isComplete ? '#34D399' : '#7C5CFC' }]}>
  ...
  return (
    <LinearGradient colors={['#09090C', '#0F0F18', '#13101E']} style={styles.root}>
      ...
      {continueBanner && (
        <Pressable
          style={({ pressed }) => [styles.continueBanner, pressed && { opacity: 0.9 }]}
        ...
        >
          <View style={styles.continueLeft}>
            ...
            <View style={styles.continueTextGroup}>
              <Text style={styles.continueBannerLabel}>{t.history_continue_banner}</Text>

...
const PARTICLE_COLORS = ['#7C5CFC', '#5DCAA5', '#A78BFA', '#7C5CFC', '#5DCAA5', '#A78BFA', '#7C5CFC', '#5DCAA5', '#A78BFA', '#7C5CFC', '#5DCAA5', '#A78BFA'];
...
function PulsingDot({ color = '#34D399' }: { color?: string }) {
  ...
  useEffect(() => {
    Animated.loop(
      Animated.sequence([
        Animated.parallel([
          Animated.timing(scale, { toValue: 1.6, duration: 1000, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
  ...
  return (
    <Animated.View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: color, opacity, transform: [{ scale }] }} />
  );
}
  ...
  return (
    <View style={{ width: 60, height: 60, alignItems: 'center', justifyContent: 'center' }}>
      <Animated.View style={{ ...StyleSheet.absoluteFillObject, transform: [{ rotate }] }}>
        <View style={{ width: 60, height: 60, borderRadius: 30, borderWidth: 1, borderColor: '#7C5CFC18' }} />
        <View style={{ position: 'absolute', top: -2.5, left: 27, width: 5, height: 5, borderRadius: 2.5, backgroundColor: '#7C5CFC40' }} />
      </Animated.View>
      <View style={{ position: 'absolute', width: 42, height: 42, borderRadius: 21, borderWidth: 0.5, borderColor: '#A78BFA10' }} />
      <View style={{ position: 'absolute', width: 24, height: 24, borderRadius: 12, borderWidth: 0.5, borderColor: '#5DCAA510' }} />
    </View>
  );
}
...
const CHAR_COLORS: Record<string, string> = {
  ENTJ: '#FF6B6B', INFP: '#4D96FF', INTJ: '#A78BFA', ENTP: '#FFD93D',
  INFJ: '#6BCB77', ISFP: '#FF9F43', ENFP: '#FF6B9D', INTP: '#74B9FF',
};
const PREVIEW_CHARS = [
  ...
  useEffect(() => {
    dots.forEach((d) => {
      const animate = () => {
        ...
        Animated.sequence([
          Animated.timing(d.opacity, { toValue: 0.2, duration: 1500, useNativeDriver: true }),
          Animated.delay(1500),
          Animated.timing(d.opacity, { toValue: 0, duration: 1500, useNativeDriver: true }),
        ]).start(() => animate());
      };
...
// ── CTA shimmer effect ──
function ShimmerCTA({ children, onPress, style }: { children: React.ReactNode; onPress: () => void; style?: any }) {
  const shimmerX = useRef(new Animated.Value(-200)).current;
  useEffect(() => {
    const run = () => {
      shimmerX.setValue(-200);
      Animated.timing(shimmerX, { toValue: SW + 200, duration: 900, easing: Easing.inOut(Easing.ease), useNativeDriver: true }).start();
    };
    run();
  ...
  return (
    <Pressable style={({ pressed }) => [style, pressed && styles.ctaPressed]} onPress={onPress}>
      <LinearGradient
...
export default function HomeScreen() {
  ...
  const { sessions, createNewSession, streak } = useSessionStore();
  const { t } = useT();
  ...
  useEffect(() => {
    charAnims.forEach((anim, i) => {
      setTimeout(() => {
        Animated.parallel([
          Animated.timing(anim.translateX, { toValue: 0, duration: 400, easing: Easing.out(Easing.ease), useNativeDriver: true }),
  ...
  const isAnimating = voice.state === 'recording' || voice.state === 'uploading' || voice.state === 'transcribing';
  useEffect(() => {
    if (isAnimating) {
      Animated.loop(
        Animated.sequence([
  ...
  return (
    <LinearGradient colors={['#09090C', '#0E0E16', '#12101D']} style={styles.root}>
      <AmbientParticles />
      ...
      >
        ...
        <View style={styles.header}>
          ...
          {streak.currentStreak > 0 && (
            <View style={styles.streakBadge}>
              <Text style={styles.streakText}>{'\u{26A1}'} {streak.currentStreak}</Text>
            </View>
          )}
        ...
        >
          {PREVIEW_CHARS.map((c, idx) => {
            ...
            const charColor = CHAR_COLORS[c.mbti] || vis?.accentColor || '#7C5CFC';
            return (
              <Animated.View key={c.mbti} style={{ opacity: anim.opacity, transform: [{ translateX: anim.translateX }] }}>
                <View style={[styles.charCard, { borderColor: charColor + '30', backgroundColor: charColor + '08' }]}>
                  <Text style={styles.charEmoji}>{vis?.emoji || c.emoji}</Text>
                  <Text style={[styles.charMbti, { color: charColor }]}>{c.mbti}</Text>
                  <Text style={styles.charRole} numberOfLines={1}>{vis?.label || c.mbti}</Text>
                </View>
              </Animated.View>
            );
          })}
        ...
        <View style={styles.inputCard}>
          ...
          {voice.state !== 'idle' && voice.state !== 'success' && voice.state !== 'error' ? (
            <View style={styles.voiceContainer}>
              <Animated.View style={[styles.voiceRing, 
              ...
              {voice.state === 'recording' && (
                <Pressable
                  style={({ pressed }) => [styles.voiceCancelBtn, pressed && { opacity: 0.7 }]}
            ...
            <View style={styles.voiceContainer}>
              ...
              <Text style={[styles.voiceListening, { color: '#EF4444' }]}>
                {voice.errorMessage ?? 'Something went wrong'}
              </Text>
              <Pressable
                style={({ pressed }) => [styles.voiceRetryBtn, pressed && { opacity: 0.7 }]}
          ...
          <View style={styles.inputFooter}>
            <Pressable
              ...
              disabled={voice.state === 'uploading' || voice.state === 'transcribing'}
            >
              <Ionicons
                name={voice.state === 'recording' ? 'stop-circle' : voice.isBusy ? 'hourglass-outline' : 'mic-outline'}
                size={16}
                color={voice.state === 'recording' ? '#EF4444' : voice.isBusy ? '#5DCAA5' : '#7C5CFC'}
              />
              <Text style={[styles.voiceToggleLabel, 
                voice.state === 'recording' && { color: '#EF4444' },
                (voice.state === 'uploading' || voice.state === 'transcribing') && { color: '#5DCAA5' },
              ]}>
                {voice.state === 'recording' ? t.home_voice_stop 
                  : voice.state === 'uploading' ? '⬆' 
                  : voice.state === 'transcribing' ? '...' 
                  : t.home_voice_write}
              </Text>
        ...
        {hasInput && (
          <View style={styles.ctaBlock}>
            ...
            <Pressable
              style={({ pressed }) => [styles.buildBtn, pressed && styles.buildBtnPressed]}
        ...
        <View style={styles.proofCard}>
          ...
          <View style={styles.proofLive}>
            <View style={styles.proofAvatars}>
              ...
              <Text style={[styles.proofAvatar, { marginLeft: -6 }]}>{'\u{1F30A}'}</Text>
              <Text style={[styles.proofAvatar, { marginLeft: -6 }]}>{'\u{1F52D}'}</Text>
              <Text style={[styles.proofAvatar, { marginLeft: -6 }]}>{'\u{1F4A1}'}</Text>
            </View>
            <TypewriterText
          ...
          <View style={styles.testimonialAuthor}>
            ...
            <View style={{ flex: 1 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                <Text style={styles.testimonialName}>Kerem A. {'\u00b7'} Istanbul</Text>
        ...
        {recentSessions.length > 0 && (
          <View style={styles.recentBlock}>
            <View style={styles.recentHeader}>
              ...
              <Pressable hitSlop={12} onPress={() => router.push('/(tabs)/gecmis')}>
                <Text style={styles.seeAll}>
                  {t.home_see_all_short}
                </Text>
              </Pressable>
            </View>
                  ...
                  <View style={styles.recentLeft}>
                    ...
                    <View style={styles.recentMeta}>
                      {session.selectedCharacters.slice(0, 3).map((mbti) => {
                        ...
                        return <Text key={mbti} style={styles.recentDot}>{vis?.emoji || '\u{1F4AC}'}</Text>;
                      })}
                      <Text style={styles.recentMetaLabel}>{session.turns.length} {t.turns_label} \u00b7 {ago}</Text>
                    </View>
                  </View>
                  <Ionicons name="chevron-forward" size={16} color="#7C5CFC60" />
                </Pressable>
              );
            })}
          </View>

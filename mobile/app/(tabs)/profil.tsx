...
function SettingsRow({ icon, label, value, onPress, isToggle, toggleValue, onToggle, danger }: SettingsRowProps) {
  return (
    <Pressable
      style={({ pressed }) => [styles.settingsRow, pressed && !isToggle && styles.settingsRowPressed]}
      onPress={onPress}
      disabled={isToggle}
    >
      <View style={styles.settingsRowLeft}>
        <Ionicons name={icon} size={18} color={danger ? '#EF444480' : '#FFFFFF40'} />
      ...
      {isToggle ? (
        ...
        <View style={styles.settingsRowRight}>
          ...
          {!danger && <Ionicons name="chevron-forward" size={14} color="#FFFFFF15" />}
        </View>
      )}
    </Pressable>
  );
  ...
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <Pressable style={styles.sheetOverlay} onPress={onClose}>
        <Animated.View
          style={[styles.sheetContent, { paddingBottom: insets.bottom + 20, transform: [{ translateY: slideAnim }] }]}
        >
...
export default function ProfileScreen() {
  ...
  const completedCount = sessions.filter((s) => s.status === 'report_ready').length;
      ...
      >
        ...
        >
          <View style={styles.accountCard}>
            ...
            <View style={styles.avatarOuter}>
              <Animated.View style={[styles.avatarGradientRing, { transform: [{ rotate }] }]}>
                <LinearGradient
        ...
        <View style={styles.settingsGroup}>
          <SettingsRow
            ...
            value={{ tr: 'Türkçe', en: 'English', ko: '한국어', ja: '日本語', pt: 'Português', es: 'Español' }[lang]}
            onPress={() => { Haptics.selectionAsync(); setLangSheetVisible(true); }}
          />

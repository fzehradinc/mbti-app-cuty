...
export default function ReportsScreen() {
  ...
  const completedSessions = sessions.filter(
    (s) => s.status === "report_ready" && s.report,
  );
  ...
  const totalVoices = completedSessions.reduce(
    (sum, s) => sum + s.selectedCharacters.length,
    0,
  );
            ...
            >
              ...
              <View style={styles.cardFooter}>
                <Text style={styles.footerMeta}>
                  {item.turns.length} {t.history_turns} ·{" "}
                  {item.selectedCharacters.length} {t.history_voices}
                </Text>
                {report?.actionPaths && (
        ...
        ListFooterComponent={
          completedSessions.length > 0 ? (
            <View style={styles.encourageBlock}>
              <Text style={styles.encourageText}>{t.report_encouragement}</Text>
              <Pressable
